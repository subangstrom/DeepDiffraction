...
layersTransfer(end+1) = dropoutLayer(0.5);%insert dropout layer after fully connected layer to reduce overfitting during training
layersTransfer(end+1) = fullyConnectedLayer(90,... % add fully connected layer based on the total number of categories.
...
inputlayer = imageInputLayer([227 227 3],'DataAugmentation',{'randcrop'});%data augumentation for random crop only
layersTransfer(1)=inputlayer;

optionsTransfer = trainingOptions('sgdm','MaxEpochs',20,... %max 20 epochs
