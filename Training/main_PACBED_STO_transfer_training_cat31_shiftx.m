clear
load('Alex_net.mat')
%demo script for PACBED_shift CNN transfer training
...
tic
digitDatasetPath_train = 'F:\STEM_pattern_data\STO_PACBED_p227_shift_14_19mrad';
...
layersTransfer(end+1) = dropoutLayer(0.5);%insert dropout layer after fully connected layer to reduce overfitting during training
layersTransfer(end+1) = fullyConnectedLayer(31,... % add fully connected layer based on the total number of categories.
...
inputlayer = imageInputLayer([227 227 3],'DataAugmentation','none'); %no data augumentation for shift and crop
layersTransfer(1)=inputlayer;

optionsTransfer = trainingOptions('sgdm','MaxEpochs',3,... %max 3 epochs
	'InitialLearnRate',0.0002,... %0.0002at beginning
...
save('try_NN_PACBED_STO_14mrad_Alexnet_cat31_shiftx.mat','convnetTransfer','optionsTransfer','digitDatasetPath_train','info')