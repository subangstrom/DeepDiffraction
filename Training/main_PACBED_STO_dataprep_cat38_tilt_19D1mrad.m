clear; %close all;clc
%demo script for PACBED_tilt(19.1mrad) CNN data prep
%Weizong Xu, July, 2017
%% get more images (generate signal images)
load('PACBED_STO_19D1mrad_data_1nm_resolution_mrad_tilt_38cat_corr3.mat','name_list','tilt_list','list_total')
img_num=1;
fpwd='F:\STEM_pattern_data\STO_PACBED_p227_AlexNet_t_cat38_4mrad_19D1mrad';
num_center=5;
...
for i_tilt=1:size(PACBED_data,1)
    load('PACBED_STO_19D1mrad_data_1nm_resolution_mrad_tilt_38cat_corr3.mat','PACBED_data')
    PACBED_data=PACBED_data(i_tilt,:);
    num=uint32(0);
    image_sum=zeros(227,227,3);
    for i_list=5:80%length(list_total) %hard to get tilt angle thinner than 5nm
        for i_size=275:15:440
    ...
    parsave(['image_sum_',num2str(i_tilt),'_cat38_t4mrad_19D1mrad.mat'],image_sum,num)
end
toc
exit
